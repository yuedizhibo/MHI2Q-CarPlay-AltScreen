...
if [ -x "$MIRROR/carplay-alt111-mirror-display" ]; then
    echo "SECOND_SCREEN_LOGO=EMBEDDED_IN_BINARY"
else
    echo "SECOND_SCREEN_LOGO=BINARY_MISSING"
    [ "$STATUS_RC" -ne 0 ] || STATUS_RC=1
fi
if [ -x "$MIRROR/carplay-alt111-mirror-display" ]; then
    echo "SECOND_SCREEN_WATERMARK=EMBEDDED_IN_BINARY asset=REFERENCE_ORIGINAL motion=BOUNCING_DRIFT"
else
    echo "SECOND_SCREEN_WATERMARK=BINARY_MISSING"
    [ "$STATUS_RC" -ne 0 ] || STATUS_RC=1
fi
